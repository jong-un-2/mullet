...
import { FaCog, FaExchangeAlt, FaLayerGroup, FaWallet, FaPiggyBank } from "react-icons/fa";
  ...
  return (
    <nav className="bg-[#1a1f2a] border-b border-[#3a4553] sticky top-0 z-50 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4">
        ...
        {/* Mobile Navigation */}
        <div className="lg:hidden border-t border-[#3a4553] py-2">
          <div className="flex items-center justify-around">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center gap-1 px-2 py-2 rounded-lg text-xs font-medium transition-all duration-200 ${
                  isActive(item.path)
