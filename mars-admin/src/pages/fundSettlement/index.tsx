import { exportuser, getBase, getChat, getcjmingxi, getgqmingxi, getjsmingxi, getrujinmingxi, getusertongji, zijinjiesuan, zijinliushui } from '@/services/ant-design-pro/api';
...
const App = () => {
    ...
    const getList = async (page = 1) => {
        ...
        setLoading(true)
        const rep = await zijinjiesuan(p)
        setLoading(false)
        if (rep.code == 200) {
    ...
    const columns = [
        {
            title: (
                <Tooltip placement="top" title="当天0点结算前一天所有币种数据">
                    日期
                </Tooltip>
            ),
            dataIndex: 'dateStr',
        ...
        {
            title: (
                <Tooltip placement="top" title="系统结余=前一日结余+入金数量+挂起数量-出金数量+出金手续费-结算数量">
                    系统结余
                </Tooltip>
            ),
        ...
        {
            title: (
                <Tooltip placement="top" title="查询主网钱包并统计该币种结余。">
                    主网结余
                </Tooltip>
            ),
