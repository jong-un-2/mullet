        ...
        case $choice in
            ...
            3)
                ...
                pool_count=$(curl -s -X POST \
                  ...
                  https://api.studio.thegraph.com/query/114739/mars-dex-bsc-testnet/version/latest \
                  -d '{"query":"{ lbpairs { id } }"}' \
    ...
    while true; do
        ...
        case $choice in
            ...
            8)
                ...
                echo "• 本地查询端点: http://localhost:8000/subgraphs/name/mars/bsc-local"
                echo "• 交互式浏览器: http://localhost:8000/subgraphs/name/mars/bsc-local/graphql"
                echo ""
                # 尝试在默认浏览器中打开
                if command -v open >/dev/null 2>&1; then
                    echo "正在打开默认浏览器..."
                    open "http://localhost:8000/subgraphs/name/mars/bsc-local/graphql"
                elif command -v xdg-open >/dev/null 2>&1; then
                    echo "正在打开默认浏览器..."
                    xdg-open "http://localhost:8000/subgraphs/name/mars/bsc-local/graphql"
                else
                    echo "请手动在浏览器中打开上述链接"
                fi
                read -p "按任意键继续..."
